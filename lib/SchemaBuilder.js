  ...
  _resolverForModel(modelData, extraQuery) {
    return (ctx, ignore1, ignore2, data) => {
      ...
      if (modelData.withPagination) {
        // unroll the AST for the nested data so that existing query logic works
    ...
    return (builder) => {
      ...
      builder.select(selects.map((it) => {
        ...
        if (jsonSchema.properties[it]) {
          if (jsonSchema.properties[it].type === 'geometry') {
            return builder.knex().raw(`ST_AsGeoJSON(${col}) AS ${col}`);
          }
